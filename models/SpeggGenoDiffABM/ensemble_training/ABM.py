...
class Loss(torch.nn.Module):
    ...
    def forward(
            ...
            true_records : torch.Tensor, 
            tstep_start : int, 
            tstep_end : int
    ):
        return ABM.apply(
            predicted_parameters, true_records, 
            tstep_start, tstep_end, self.loss_func, 
            self.epsilon)

class ABM(torch.autograd.Function):
    """
    This class approximates the derivatives of all parameters, 
    since autodifferentiation gets disconnected when it reaches 
    spegg program (which is not a part of torch ecosystem)
    """
    @staticmethod
    def forward(
        ctx, 
    ...
    def backward(ctx, grad_outputs):
        ...
        return grad_outputs * grad_tensor, None, None, None, None, None
