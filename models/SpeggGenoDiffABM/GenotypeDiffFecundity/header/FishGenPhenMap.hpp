...
/**
 * @brief This model basically amplifies the effect of geneotype locus 0 on the 
 * fecundity phenotype by multiplying the allelic values across two sets of alleles 
 * by 3, while the effect of genotype locus 1 on the fecundity phenotype is not amplified.
 * 
 */
...
class fin_color_phenotype : public GenotypePhenotypeMap
{
public:
    int FIN_COLOR_INDEX;

    fin_color_phenotype(inds *species, int phenotype_index, int index_case, int num_kids) : GenotypePhenotypeMap(species, phenotype_index, index_case, num_kids)
    {
        this->FIN_COLOR_INDEX = this->phenotype_index;
    };

    void calculate_phenotype(inds *species);
};

struct fin_color_calculator
{
    float *map_constant;
    float *map_coefficient_0;

    fin_color_calculator(float *map_cons, float *map_coef0) : map_constant(map_cons), map_coefficient_0(map_coef0) {};
    ...
    1: individual's maternally inherited allelic value at their locus 0
    2: individual's paternally inherited allelic value at their locus 0
    3: individual's fin color phenotypic value
    */
