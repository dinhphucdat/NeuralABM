...
#### `SIR_Langevin_sampler` in [`Langevin.py`](/models/SIR/ensemble_training/Langevin.py)

Alternative to training data using a Neural Network is to use Stochastic Gradient Langevin Descent method as a numerical optimization strategy with some random noise that will bounce the parameters back and forth to get them out of the local optima. Similar to Neural Network method, this training style also takes in dataset generated by the Agent-Based simulation process and then will make a guess of those three parameters, calculate the loss and prefix with some random noise, optimize, until it gets to the set of optimized parameters.
